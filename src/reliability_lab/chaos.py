...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    ...
    for breaker in gateway.breakers.values():
        ...
        for t in breaker.transition_log:
            if t.get("to") == "open":
                open_ts = float(t["ts"])
            elif t.get("to") == "closed" and open_ts is not None:
                recovery_times.append((float(t["ts"]) - open_ts) * 1000.0)
                open_ts = None
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    metrics.circuit_open_count = sum(
        sum(1 for t in b.transition_log if t.get("to") == "open") for b in gateway.breakers.values()
    )
